...
from dodobot_tools.joystick import Joystick
...
class TJ2DebugJoystick:
    ...
    def joystick_msg_callback(self, msg):
        ...
        elif self.joystick.did_button_down(("triggers", "L1")):
        ...
        if self.enable_tilt_axis:
            ...
            self.set_linear(self.joystick.get_axis(self.stepper_axis))

        if any(self.joystick.check_list(self.joystick.did_axis_change, self.linear_axis, self.angular_axis)):
            self.disable_timer = rospy.Time.now()
            linear_val = self.joystick.deadband_axis(self.linear_axis, self.deadzone_joy_val, self.linear_x_scale)

...
from dodobot_serial_bridge.msg import DodoBotServos
from dodobot_serial_bridge.srv import DodoBotMenuSrv
...
class ParsingJoystick:
    def __init__(self):
        ...
        self.servo_command = DodoBotServos()
        ...
        self.servo_pub = rospy.Publisher("servos", DodoBotServos, queue_size=100)
        ...
        self.camera_command_mode = DodoBotServos.POSITION_MODE

        self.cmd_vel_timeout = rospy.Time.now()
        # rospy.Timer(rospy.Duration(0.25), self.timer_callback)

        self.menu_service_name = "dodobot_menu"
        self.menu_events = {
            "up": "^",
            "down": "v",
            "left": "<",
        ...
        self.send_menu_event = rospy.ServiceProxy(self.menu_service_name, DodoBotMenuSrv)
    ...
    def set_servos(self, camera_pan_val=None, camera_tilt_val=None, mode=DodoBotServos.POSITION_MODE):
        ...
        if mode != self.servo_command.mode:
            if mode == DodoBotServos.POSITION_MODE:
                rospy.loginfo("Switching to camera turret to position mode")
            elif mode == DodoBotServos.VELOCITY_MODE:
    ...
    def process_joy_msg(self, msg):
        ...
        if self.camera_command_mode == DodoBotServos.VELOCITY_MODE:
        ...
        else:  # DodoBotServos.POSITION_MODE
        ...
        if self.did_button_change(msg, 0):  # A
            ...
            if self.camera_command_mode == DodoBotServos.POSITION_MODE:
                self.camera_command_mode = DodoBotServos.VELOCITY_MODE
            else:
                self.camera_command_mode = DodoBotServos.POSITION_MODE

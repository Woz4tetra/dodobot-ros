...
class Pose2d:
    ...
    def from_ros_pose(cls, pose):
        ...
        self.theta = Pose2d.theta_from_quat(pose.orientation)
        return self
    ...
    def clip(self, lower, upper):
        ...
        if self.magnitude() < lower.magnitude():
            state.theta = Pose2d._clip(self.theta, lower.theta, upper.theta)
        if abs(self.theta) < lower.theta:
            state.x = Pose2d._clip(self.x, lower.x, upper.x)
            state.y = Pose2d._clip(self.y, lower.y, upper.y)
        return state

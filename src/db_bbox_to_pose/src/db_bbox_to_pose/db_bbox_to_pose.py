...
class DodobotBboxToPose:
    ...
    def __init__(self):
        ...
        self.marker_persistance = rospy.get_param("~marker_persistance_s", 1.0 / 10.0)
    ...
    def depth_detect_callback(self, depth_msg, detect_msg):
        ...
        for detection in detect_msg.detections:
            ...
            pose, scale_vector = self.bbox_to_pose(detection.bbox, cv_image, label_name)
            ...
            text_marker = self.make_marker(pose, label_name, label_index, scale_vector)

            sphere_marker.type = Marker.SPHERE
            text_marker.type = Marker.TEXT_VIEW_FACING
            text_marker.text = "%s_%s" % (label, label_index)
    ...
    def bbox_to_pose(self, bbox, depth_image, label):
        ...
        z_dist = self.get_z_dist(depth_image, x_center, y_center, detect_radius, self.publish_debug_image)
        ...
        scale_vector.x = abs(x_obj_dist - x_dist)
        scale_vector.y = abs(y_obj_dist - y_dist)
        scale_vector.z = 0.05  # no measurement for Z depth, picking arbitrary value
    ...
    def get_z_dist(self, depth_image, x_center, y_center, detect_radius, draw_debug=True):

    ...
    _image_transport(nh)
{
    ...
    sync->registerCallback(boost::bind(&DodobotObjectFilter::rgbd_callback, this, _1, _2, _3));

    ROS_INFO("db_object_filter init done");
}


void DodobotObjectFilter::load_detectnet_model()
{
    /*
...
void DodobotObjectFilter::load_labels()
{
    /*
...
void DodobotObjectFilter::rgbd_callback(
...
ObjPoseDescription DodobotObjectFilter::bbox_to_pose(cv::Mat depth_cv_image, vision_msgs::BoundingBox2D bbox, ros::Time stamp, string label, int label_index)
{
...
visualization_msgs::Marker DodobotObjectFilter::make_marker(ObjPoseDescription* desc)
{
...
double DodobotObjectFilter::get_z_dist(cv::Mat depth_cv_image, ObjPoseDescription* desc)
{
...
// classify the image
int DodobotObjectFilter::detect(const ImageConstPtr& color_image, vision_msgs::Detection2DArray* msg)
{
...
void DodobotObjectFilter::reset_label_counter()
{
...
bool DodobotObjectFilter::publish_overlay(detectNet::Detection* detections, int num_detections)
{

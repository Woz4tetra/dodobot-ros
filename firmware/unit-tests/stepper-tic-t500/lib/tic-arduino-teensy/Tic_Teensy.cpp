...
void TicI2C::commandW32(TicCommand cmd, uint32_t val)
{
  _i2c->beginTransmission(_address);
  _i2c->write((uint8_t)cmd);
  _i2c->write(val >> 0); // lowest byte
  _i2c->write(val >> 8);
  _i2c->write(val >> 16);
  _i2c->write(val >> 24); // highest byte
  _lastError = _i2c->endTransmission();
}
  ...
  uint8_t length, void * buffer)
{
  ...
  _i2c->write((uint8_t)cmd);
  _i2c->write(offset);
  _lastError = _i2c->endTransmission(false); // no stop (repeated start)
  if (_lastError)
  {
    // Set the buffer bytes to 0 so the program will not use an uninitialized
    // variable.
  ...
  uint8_t byteCount = _i2c->requestFrom(_address, (uint8_t)length);
  if (byteCount != length)
  {
    _lastError = 50;
    memset(buffer, 0, length);
